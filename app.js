...
// Mode de l'application : 'referent' (par défaut) ou 'tech'
const APP_MODE = (document.body && document.body.dataset && document.body.dataset.mode) ? document.body.dataset.mode : 'referent';
if (APP_MODE === 'tech') state.view = 'brief';
...
// Extrait une date YYYY-MM-DD depuis un nom de fichier type JOURNEE_2026-02-01_XXX.pdf
...
function updateDayInfo() {
  const el = $("dayInfo");
  if (!el) return;
  const d = state.dayDate || extractDateFromFilename(state.pdfName) || "—";
  const btCount = (state.bts && state.bts.length) ? state.bts.length : 0;
  const docCount = (state.bts && state.bts.length) ? state.bts.reduce((a,b)=>a+(b.docs?b.docs.length:0),0) : 0;
  el.innerHTML = `<b>${d}</b><div style="margin-top:4px;">BT : ${btCount} · Docs : ${docCount}</div>`;
}
...
function renderBrief(filtered) {
  ...
  // brief => côté technicien : pas de sélection, on affiche tout
  if (APP_MODE !== 'tech' && !state.filters.techId) {
  ...
  if (meta) {
    if (APP_MODE === 'tech') {
      const d = state.dayDate || extractDateFromFilename(state.pdfName);
      meta.textContent = (d ? `Journée : ${d} — ` : "") + `${filtered.length} BT`;
    } else {
      meta.textContent = t ? `${t.name} — ${filtered.length} BT` : "";
    }
  }
  ...
  if (input) {
    input.addEventListener("change", async () => {
      ...
      try {
        ...
        state.dayDate = extractDateFromFilename(state.pdfName) || "";
        updateDayInfo();
